...
[[nodiscard]] constexpr static inline guids_2_t make_guids(void)
{
	guids_2_t guids;

	guids = guids_2_get_all();
	return guids;
}
...
public:
	enum string_id : std::size_t
	{
		...
		mk_x_dlls_to_load()
		#undef x
		#define x(d1, d2, d3, b1, b2, b3, b4, b5, b6, b7, b8, name) xx(name, #name)
		mk_x_guids_2()
		#undef x
		#undef xx
	};
...
[[nodiscard]] static inline mk_view_wstr_t wstr_to_wstr(LPCWSTR const win_str)
{
	...
	{
		wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_empty));
	}
	mk_assert(wstr.m_len >= 0);
	mk_assert(wstr.m_buf[wstr.m_len] == L'\0');
	return wstr;
...
static inline void mkfw_load_all(PPEB const peb)
{
	...
	#define x(name) g_app.m_dlls.m_##name = g_app.m_funcs_kernel.m_pfn_LoadLibraryExA(nstr_to_nstr(k_strings.get_nstr(k_strings.string_id::id_##name)).m_buf, NULL, LOAD_LIBRARY_SEARCH_SYSTEM32); mk_assert(g_app.m_dlls.m_##name);
	mk_x_dlls_to_load()
	#undef x
...
[[nodiscard]] static inline mk_view_wstr_t guid_to_text(GUID const* const guid)
{
	...
	if(guid)
	{
		...
		if(i != n)
		{
			wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_FWPM_CALLOUT_BUILT_IN_RESERVED_1 + i));
		}
		else
		{
	...
	{
		wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_none));
	}
	mk_assert(wstr.m_len >= 0);
	mk_assert(wstr.m_buf[wstr.m_len] == L'\0');
	return wstr;
...
[[nodiscard]] static inline bool guid_is_null(GUID const* const guid)
{
	GUID guid_null;
	bool eq;
	bool is;
...
[[nodiscard]] static inline mk_view_nstr_t value_to_nstr_uint8(UINT8 const u8)
{
	...
	mk_memcpy_c(&fmt[0], k_strings.get_nstr(k_strings.string_id::id_fmt_u8).m_buf, k_strings.get_nstr(k_strings.string_id::id_fmt_u8).m_len);
	fmt[k_strings.get_nstr(k_strings.string_id::id_fmt_u8).m_len] = '\0';
...
[[nodiscard]] static inline mk_view_nstr_t value_to_nstr_uint16(UINT16 const u16)
{
	...
	buf = &g_app.m_tmp_nstrs[g_app.m_tmps_nstr_idx++ % _countof(g_app.m_tmp_nstrs)][0];
	cap = _countof(g_app.m_tmp_nstrs[0]);
	mk_memcpy_c(&fmt[0], k_strings.get_nstr(k_strings.string_id::id_fmt_u16).m_buf, k_strings.get_nstr(k_strings.string_id::id_fmt_u16).m_len);
	fmt[k_strings.get_nstr(k_strings.string_id::id_fmt_u16).m_len] = '\0';
...
[[nodiscard]] static inline mk_view_nstr_t value_to_nstr_uint32(UINT32 const u32)
{
	...
	buf = &g_app.m_tmp_nstrs[g_app.m_tmps_nstr_idx++ % _countof(g_app.m_tmp_nstrs)][0];
	cap = _countof(g_app.m_tmp_nstrs[0]);
	mk_memcpy_c(&fmt[0], k_strings.get_nstr(k_strings.string_id::id_fmt_u32).m_buf, k_strings.get_nstr(k_strings.string_id::id_fmt_u32).m_len);
	fmt[k_strings.get_nstr(k_strings.string_id::id_fmt_u32).m_len] = '\0';
...
[[nodiscard]] static inline mk_view_nstr_t value_to_nstr_uint64(UINT64 const* const u64)
{
	...
	buf = &g_app.m_tmp_nstrs[g_app.m_tmps_nstr_idx++ % _countof(g_app.m_tmp_nstrs)][0];
	cap = _countof(g_app.m_tmp_nstrs[0]);
	mk_memcpy_c(&fmt[0], k_strings.get_nstr(k_strings.string_id::id_fmt_u64).m_buf, k_strings.get_nstr(k_strings.string_id::id_fmt_u64).m_len);
	fmt[k_strings.get_nstr(k_strings.string_id::id_fmt_u64).m_len] = '\0';
...
[[nodiscard]] static inline mk_view_nstr_t value_to_nstr_arr16(FWP_BYTE_ARRAY16 const* const arr16)
{
	...
	buf = &g_app.m_tmp_nstrs[g_app.m_tmps_nstr_idx++ % _countof(g_app.m_tmp_nstrs)][0];
	cap = _countof(g_app.m_tmp_nstrs[0]);
	mk_memcpy_c(&fmt[0], k_strings.get_nstr(k_strings.string_id::id_fmt_arr16).m_buf, k_strings.get_nstr(k_strings.string_id::id_fmt_arr16).m_len);
	fmt[k_strings.get_nstr(k_strings.string_id::id_fmt_arr16).m_len] = '\0';
...
[[nodiscard]] static inline mk_view_wstr_t value_to_wstr_range(FWP_RANGE0 const* const range)
{
	...
	txt_from = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_range_from));
	txt_to = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_range_to));
	buf = &g_app.m_tmp_wstrs[g_app.m_tmps_wstr_idx++ % _countof(g_app.m_tmp_wstrs)][0];
	ptr = buf;
...
[[nodiscard]] static inline mk_view_wstr_t entry_to_wstr_line(FWPM_FILTER0 const* const filter)
{
	...
	for(i = 0; i != n; ++i)
	{
		...
		wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_tab));
...
[[nodiscard]] static inline mk_view_nstr_t ip_address_v4_to_nstr(UINT32 const u32)
{
	...
	buf = &g_app.m_tmp_nstrs[g_app.m_tmps_nstr_idx++ % _countof(g_app.m_tmp_nstrs)][0];
	cap = _countof(g_app.m_tmp_nstrs[0]);
	mk_memcpy_c(&fmt[0], k_strings.get_nstr(k_strings.string_id::id_fmt_ipv4).m_buf, k_strings.get_nstr(k_strings.string_id::id_fmt_ipv4).m_len);
	fmt[k_strings.get_nstr(k_strings.string_id::id_fmt_ipv4).m_len] = '\0';
...
[[nodiscard]] static inline mk_view_nstr_t ip_address_v6_range_to_nstr(FWP_CONDITION_VALUE0 const* const range)
{
	...
	else if(same_bits > 16 && same_bits <= 32)
	{
		...
		buf = &g_app.m_tmp_nstrs[g_app.m_tmps_nstr_idx++ % _countof(g_app.m_tmp_nstrs)][0];
		cap = _countof(g_app.m_tmp_nstrs[0]);
		mk_memcpy_c(&fmt[0], k_strings.get_nstr(k_strings.string_id::id_fmt_ipv6_mask_32).m_buf, k_strings.get_nstr(k_strings.string_id::id_fmt_ipv6_mask_32).m_len);
		fmt[k_strings.get_nstr(k_strings.string_id::id_fmt_ipv6_mask_32).m_len] = '\0';
	...
	else if(same_bits > 32 && same_bits <= 48)
	{
		...
		buf = &g_app.m_tmp_nstrs[g_app.m_tmps_nstr_idx++ % _countof(g_app.m_tmp_nstrs)][0];
		cap = _countof(g_app.m_tmp_nstrs[0]);
		mk_memcpy_c(&fmt[0], k_strings.get_nstr(k_strings.string_id::id_fmt_ipv6_mask_48).m_buf, k_strings.get_nstr(k_strings.string_id::id_fmt_ipv6_mask_48).m_len);
		fmt[k_strings.get_nstr(k_strings.string_id::id_fmt_ipv6_mask_48).m_len] = '\0';
	...
	else if(same_bits > 112 && same_bits <= 128)
	{
		...
		buf = &g_app.m_tmp_nstrs[g_app.m_tmps_nstr_idx++ % _countof(g_app.m_tmp_nstrs)][0];
		cap = _countof(g_app.m_tmp_nstrs[0]);
		mk_memcpy_c(&fmt[0], k_strings.get_nstr(k_strings.string_id::id_fmt_ipv6_mask_128).m_buf, k_strings.get_nstr(k_strings.string_id::id_fmt_ipv6_mask_128).m_len);
		fmt[k_strings.get_nstr(k_strings.string_id::id_fmt_ipv6_mask_128).m_len] = '\0';
		arr16_to_arr8(&range->rangeValue->valueLow.byteArray16->byteArray16[0], &parts[0]);
	...
	{
		nstr = nstr_to_nstr(k_strings.get_nstr(k_strings.string_id::id_empty));
	}
	mk_assert(nstr.m_len >= 0);
	mk_assert(nstr.m_buf[nstr.m_len] == '\0');
	return nstr;
...
[[nodiscard]] static inline mk_view_nstr_t ip_address_v4_range_to_nstr(FWP_CONDITION_VALUE0 const* const range)
{
	...
	{
		nstr = nstr_to_nstr(k_strings.get_nstr(k_strings.string_id::id_every_ipv4));
		buf = &g_app.m_tmp_nstrs[g_app.m_tmps_nstr_idx++ % _countof(g_app.m_tmp_nstrs)][0];
		cap = _countof(g_app.m_tmp_nstrs[0]);
		fmt = nstr_to_nstr(k_strings.get_nstr(k_strings.string_id::id_fmt_ipv4_mask_08));
	...
	else if(same_bits > 8 && same_bits <= 16)
	{
		buf = &g_app.m_tmp_nstrs[g_app.m_tmps_nstr_idx++ % _countof(g_app.m_tmp_nstrs)][0];
		cap = _countof(g_app.m_tmp_nstrs[0]);
		fmt = nstr_to_nstr(k_strings.get_nstr(k_strings.string_id::id_fmt_ipv4_mask_16));
		u32_to_arr4(range->rangeValue->valueHigh.uint32, &parts_a[0]);
		u32_to_arr4(range->rangeValue->valueLow .uint32, &parts_c[0]);
		u32_to_arr4(range->rangeValue->valueHigh.uint32, &parts_d[0]);
		parts_b[0] = parts_a[0]; parts_b[1] = parts_a[1]; parts_b[2] = parts_a[2]; parts_b[3] = parts_a[3];
	...
	else if(same_bits > 16 && same_bits <= 24)
	{
		buf = &g_app.m_tmp_nstrs[g_app.m_tmps_nstr_idx++ % _countof(g_app.m_tmp_nstrs)][0];
		cap = _countof(g_app.m_tmp_nstrs[0]);
		fmt = nstr_to_nstr(k_strings.get_nstr(k_strings.string_id::id_fmt_ipv4_mask_24));
		u32_to_arr4(range->rangeValue->valueHigh.uint32, &parts_a[0]);
		u32_to_arr4(range->rangeValue->valueLow .uint32, &parts_c[0]);
		u32_to_arr4(range->rangeValue->valueHigh.uint32, &parts_d[0]);
		parts_b[0] = parts_a[0]; parts_b[1] = parts_a[1]; parts_b[2] = parts_a[2]; parts_b[3] = parts_a[3];
	...
	else if(same_bits > 24 && same_bits <= 32)
	{
		buf = &g_app.m_tmp_nstrs[g_app.m_tmps_nstr_idx++ % _countof(g_app.m_tmp_nstrs)][0];
		cap = _countof(g_app.m_tmp_nstrs[0]);
		fmt = nstr_to_nstr(k_strings.get_nstr(k_strings.string_id::id_fmt_ipv4_mask_32));
		u32_to_arr4(range->rangeValue->valueHigh.uint32, &parts_a[0]);
		u32_to_arr4(range->rangeValue->valueLow .uint32, &parts_c[0]);
		u32_to_arr4(range->rangeValue->valueHigh.uint32, &parts_d[0]);
		parts_b[0] = parts_a[0]; parts_b[1] = parts_a[1]; parts_b[2] = parts_a[2]; parts_b[3] = parts_a[3];
...
[[nodiscard]] static inline mk_view_wstr_t condition_entry_to_wstr_note(FWPM_FILTER0 const* const filter, FWPM_FILTER_CONDITION0 const* const condition)
{
	...
	wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_empty));
	if(false){}
	else if((guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_PROTOCOL])) && (condition->conditionValue.type == FWP_UINT8) && (condition->conditionValue.uint8 ==   1)){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_protocol_icmpv4)     ); }
	else if((guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_PROTOCOL])) && (condition->conditionValue.type == FWP_UINT8) && (condition->conditionValue.uint8 ==   2)){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_protocol_igmp)       ); }
	else if((guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_PROTOCOL])) && (condition->conditionValue.type == FWP_UINT8) && (condition->conditionValue.uint8 ==   6)){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_protocol_tcp)        ); }
	else if((guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_PROTOCOL])) && (condition->conditionValue.type == FWP_UINT8) && (condition->conditionValue.uint8 ==  17)){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_protocol_udp)        ); }
	else if((guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_PROTOCOL])) && (condition->conditionValue.type == FWP_UINT8) && (condition->conditionValue.uint8 ==  41)){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_protocol_ipv6generic)); }
	else if((guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_PROTOCOL])) && (condition->conditionValue.type == FWP_UINT8) && (condition->conditionValue.uint8 ==  43)){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_protocol_ipv6route)  ); }
	else if((guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_PROTOCOL])) && (condition->conditionValue.type == FWP_UINT8) && (condition->conditionValue.uint8 ==  44)){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_protocol_ipv6frag)   ); }
	else if((guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_PROTOCOL])) && (condition->conditionValue.type == FWP_UINT8) && (condition->conditionValue.uint8 ==  47)){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_protocol_gre)        ); }
	else if((guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_PROTOCOL])) && (condition->conditionValue.type == FWP_UINT8) && (condition->conditionValue.uint8 ==  58)){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_protocol_icmpv6)     ); }
	else if((guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_PROTOCOL])) && (condition->conditionValue.type == FWP_UINT8) && (condition->conditionValue.uint8 ==  59)){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_protocol_ipv6nonxt)  ); }
	else if((guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_PROTOCOL])) && (condition->conditionValue.type == FWP_UINT8) && (condition->conditionValue.uint8 ==  60)){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_protocol_ipv6opts)   ); }
	else if((guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_PROTOCOL])) && (condition->conditionValue.type == FWP_UINT8) && (condition->conditionValue.uint8 == 112)){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_protocol_vrrp)       ); }
	else if((guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_PROTOCOL])) && (condition->conditionValue.type == FWP_UINT8) && (condition->conditionValue.uint8 == 113)){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_protocol_pgm)        ); }
	else if((guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_PROTOCOL])) && (condition->conditionValue.type == FWP_UINT8) && (condition->conditionValue.uint8 == 115)){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_protocol_l2tp)       ); }
	else if((guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_REMOTE_ADDRESS])) && (condition->conditionValue.type == FWP_UINT32)){ wstr = nstr_to_wstr(ip_address_v4_to_nstr(condition->conditionValue.uint32)); }
	else if((guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_REMOTE_ADDRESS])) && (condition->conditionValue.type == FWP_RANGE_TYPE) && (condition->conditionValue.rangeValue->valueLow.type == FWP_BYTE_ARRAY16_TYPE) && (condition->conditionValue.rangeValue->valueHigh.type == FWP_BYTE_ARRAY16_TYPE)){ wstr = nstr_to_wstr(ip_address_v6_range_to_nstr(&condition->conditionValue)); }
	...
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V4    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_packet_too_big         )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_packet_too_big)         ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V4])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_packet_too_big         )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_packet_too_big)         ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V4  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_packet_too_big         )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_packet_too_big)         ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V4 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_packet_too_big         )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_packet_too_big)         ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V4    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_destination_unreachable)){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_destination_unreachable)); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V4])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_destination_unreachable)){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_destination_unreachable)); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V4  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_destination_unreachable)){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_destination_unreachable)); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V4 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_destination_unreachable)){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_destination_unreachable)); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V4    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_source_quench          )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_source_quench)          ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V4])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_source_quench          )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_source_quench)          ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V4  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_source_quench          )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_source_quench)          ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V4 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_source_quench          )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_source_quench)          ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V4    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_redirect               )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_redirect)               ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V4])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_redirect               )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_redirect)               ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V4  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_redirect               )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_redirect)               ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V4 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_redirect               )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_redirect)               ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V4    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_echo_request           )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_echo_request)           ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V4])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_echo_request           )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_echo_request)           ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V4  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_echo_request           )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_echo_request)           ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V4 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_echo_request           )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_echo_request)           ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V4    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_router_advertisement   )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_router_advertisement)   ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V4])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_router_advertisement   )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_router_advertisement)   ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V4  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_router_advertisement   )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_router_advertisement)   ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V4 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_router_advertisement   )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_router_advertisement)   ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V4    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_router_solicitation    )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_router_solicitation)    ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V4])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_router_solicitation    )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_router_solicitation)    ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V4  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_router_solicitation    )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_router_solicitation)    ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V4 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_router_solicitation    )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_router_solicitation)    ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V4    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_time_exceeded          )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_time_exceeded)          ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V4])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_time_exceeded          )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_time_exceeded)          ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V4  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_time_exceeded          )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_time_exceeded)          ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V4 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_time_exceeded          )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_time_exceeded)          ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V4    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_parameter_problem      )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_parameter_problem)      ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V4])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_parameter_problem      )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_parameter_problem)      ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V4  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_parameter_problem      )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_parameter_problem)      ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V4 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_parameter_problem      )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_parameter_problem)      ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V4    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_timestamp_request      )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_timestamp_request)      ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V4])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_timestamp_request      )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_timestamp_request)      ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V4  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_timestamp_request      )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_timestamp_request)      ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V4 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_timestamp_request      )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_timestamp_request)      ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V4    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_address_mask_request   )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_address_mask_request)   ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V4])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_address_mask_request   )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_address_mask_request)   ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V4  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_address_mask_request   )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_address_mask_request)   ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V4 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v4_e_address_mask_request   )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_address_mask_request)   ); }

	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V6    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_destination_unreachable         )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_destination_unreachable)         ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V6])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_destination_unreachable         )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_destination_unreachable)         ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V6  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_destination_unreachable         )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_destination_unreachable)         ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V6 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_destination_unreachable         )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_destination_unreachable)         ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V6    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_packet_too_big                  )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_packet_too_big)                  ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V6])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_packet_too_big                  )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_packet_too_big)                  ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V6  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_packet_too_big                  )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_packet_too_big)                  ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V6 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_packet_too_big                  )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_packet_too_big)                  ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V6    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_time_exceeded                   )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_time_exceeded)                   ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V6])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_time_exceeded                   )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_time_exceeded)                   ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V6  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_time_exceeded                   )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_time_exceeded)                   ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V6 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_time_exceeded                   )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_time_exceeded)                   ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V6    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_parameter_problem               )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_parameter_problem)               ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V6])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_parameter_problem               )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_parameter_problem)               ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V6  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_parameter_problem               )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_parameter_problem)               ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V6 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_parameter_problem               )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_parameter_problem)               ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V6    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_echo_request                    )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_echo_request)                    ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V6])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_echo_request                    )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_echo_request)                    ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V6  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_echo_request                    )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_echo_request)                    ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V6 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_echo_request                    )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_echo_request)                    ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V6    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_multicast_listener_query        )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_multicast_listener_query)        ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V6])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_multicast_listener_query        )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_multicast_listener_query)        ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V6  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_multicast_listener_query        )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_multicast_listener_query)        ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V6 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_multicast_listener_query        )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_multicast_listener_query)        ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V6    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_multicast_listener_report       )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_multicast_listener_report)       ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V6])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_multicast_listener_report       )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_multicast_listener_report)       ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V6  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_multicast_listener_report       )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_multicast_listener_report)       ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V6 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_multicast_listener_report       )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_multicast_listener_report)       ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V6    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_multicast_listener_done         )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_multicast_listener_done)         ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V6])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_multicast_listener_done         )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_multicast_listener_done)         ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V6  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_multicast_listener_done         )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_multicast_listener_done)         ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V6 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_multicast_listener_done         )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_multicast_listener_done)         ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V6    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_router_solicitation             )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_router_solicitation)             ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V6])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_router_solicitation             )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_router_solicitation)             ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V6  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_router_solicitation             )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_router_solicitation)             ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V6 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_router_solicitation             )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_router_solicitation)             ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V6    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_router_advertisement            )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_router_advertisement)            ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V6])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_router_advertisement            )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_router_advertisement)            ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V6  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_router_advertisement            )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_router_advertisement)            ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V6 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_router_advertisement            )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_router_advertisement)            ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V6    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_neighbor_discovery_solicitation )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_neighbor_discovery_solicitation) ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V6])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_neighbor_discovery_solicitation )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_neighbor_discovery_solicitation) ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V6  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_neighbor_discovery_solicitation )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_neighbor_discovery_solicitation) ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V6 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_neighbor_discovery_solicitation )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_neighbor_discovery_solicitation) ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V6    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_neighbor_discovery_advertisement)){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_neighbor_discovery_advertisement)); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V6])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_neighbor_discovery_advertisement)){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_neighbor_discovery_advertisement)); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V6  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_neighbor_discovery_advertisement)){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_neighbor_discovery_advertisement)); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V6 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_neighbor_discovery_advertisement)){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_neighbor_discovery_advertisement)); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V6    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_redirect                        )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_redirect)                        ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V6])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_redirect                        )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_redirect)                        ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V6  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_redirect                        )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_redirect)                        ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V6 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_redirect                        )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_redirect)                        ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_CONNECT_V6    ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_multicast_listener_report_v2    )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_multicast_listener_report_v2)    ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V6])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_ORIGINAL_ICMP_TYPE])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_multicast_listener_report_v2    )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_multicast_listener_report_v2)    ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_INBOUND_ICMP_ERROR_V6  ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_multicast_listener_report_v2    )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_multicast_listener_report_v2)    ); }
	else if((guid_eq(&filter->layerKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_LAYER_OUTBOUND_ICMP_ERROR_V6 ])) && (guid_eq(&condition->fieldKey, &k_konst.m_guids.m_guids[guid_id_e_FWPM_CONDITION_IP_LOCAL_PORT     ])) && (condition->matchType == FWP_MATCH_EQUAL) && (condition->conditionValue.type == FWP_UINT16) && (condition->conditionValue.uint16 == mk_net_icmp_v6_e_multicast_listener_report_v2    )){ wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_icmp_multicast_listener_report_v2)    ); }
...
[[nodiscard]] static inline mk_view_wstr_t condition_entry_to_wstr_table(FWPM_FILTER0* const entry)
{
	...
	for(i = 0; i != n; ++i)
	{
		...
		wstr = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_nl));
		mk_assert(len + ((int)(wstr.m_len)) < cap);
		ptr = mk_memcpy(buf + len, wstr); ((void)(ptr));
		len += ((int)(wstr.m_len));
	}
...
static inline void mkfw_wnd_on_key_apps(mk_wnd_t* const self, HWND const list_view)
{
	...
	mi.dwTypeData = ((LPWSTR)(nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_menu_copy_line)).m_buf));
	...
	mi.dwTypeData = ((LPWSTR)(nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_menu_copy_table)).m_buf));
...
static inline void mkfw_wnd_on_rclick(mk_wnd_t* const self, HWND const list_view, LPARAM const lparam)
{
	...
	mi.dwTypeData = ((LPWSTR)(nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_menu_copy_value)).m_buf));
...
static inline void mkfw_wnd__proc__create(mk_wnd_t* const self, HWND const hwnd, UINT const msg, WPARAM const wparam, LPARAM const lparam, bool* const out_call_def, LRESULT* const out_lr)
{
	...
	self->m_entries = g_app.m_funcs_user.m_pfn_CreateWindowExW(WS_EX_LEFT | WS_EX_LTRREADING | WS_EX_RIGHTSCROLLBAR, nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_wnd_cls_name_list_view)).m_buf, nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_empty)).m_buf, WS_VISIBLE | WS_CHILD | LVS_REPORT | LVS_OWNERDATA | LVS_SINGLESEL | LVS_SHOWSELALWAYS, 10, 10, 800, 600, self->m_hwnd, NULL, g_app.m_dlls.m_exe, NULL); mk_assert(self->m_entries);
	...
	col.mask = LVCF_WIDTH | LVCF_TEXT; col.pszText = ((LPWSTR)(nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_filter)).m_buf)); col.cx = 80;
	...
	col.mask = LVCF_WIDTH | LVCF_TEXT; col.pszText = ((LPWSTR)(nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_name)).m_buf)); col.cx = 300;
	...
	col.mask = LVCF_WIDTH | LVCF_TEXT; col.pszText = ((LPWSTR)(nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_description)).m_buf)); col.cx = 180;
	...
	col.mask = LVCF_WIDTH | LVCF_TEXT; col.pszText = ((LPWSTR)(nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_provider)).m_buf)); col.cx = 80;
	...
	col.mask = LVCF_WIDTH | LVCF_TEXT; col.pszText = ((LPWSTR)(nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_layer)).m_buf)); col.cx = 80;
	...
	col.mask = LVCF_WIDTH | LVCF_TEXT; col.pszText = ((LPWSTR)(nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_sublayer)).m_buf)); col.cx = 80;
	...
	col.mask = LVCF_WIDTH | LVCF_TEXT; col.pszText = ((LPWSTR)(nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_action)).m_buf)); col.cx = 80;
	...
	col.mask = LVCF_WIDTH | LVCF_TEXT; col.pszText = ((LPWSTR)(nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_filter_type_callout)).m_buf)); col.cx = 80;
	...
	col.mask = LVCF_WIDTH | LVCF_TEXT; col.pszText = ((LPWSTR)(nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_filter_id)).m_buf)); col.cx = 80;
	...
	col.mask = LVCF_WIDTH | LVCF_TEXT; col.pszText = ((LPWSTR)(nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_weight)).m_buf)); col.cx = 80;
	...
	col.mask = LVCF_WIDTH | LVCF_TEXT; col.pszText = ((LPWSTR)(nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_effective_weight)).m_buf)); col.cx = 80;
	...
	self->m_conditions = g_app.m_funcs_user.m_pfn_CreateWindowExW(WS_EX_LEFT | WS_EX_LTRREADING | WS_EX_RIGHTSCROLLBAR, nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_wnd_cls_name_list_view)).m_buf, nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_empty)).m_buf, WS_VISIBLE | WS_CHILD | LVS_REPORT | LVS_OWNERDATA | LVS_SINGLESEL | LVS_SHOWSELALWAYS, 10, 10, 800, 600, self->m_hwnd, NULL, g_app.m_dlls.m_exe, NULL); mk_assert(self->m_entries);
	...
	col.mask = LVCF_WIDTH | LVCF_TEXT; col.pszText = ((LPWSTR)(nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_field)).m_buf)); col.cx = 80;
	...
	col.mask = LVCF_WIDTH | LVCF_TEXT; col.pszText = ((LPWSTR)(nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_match_type)).m_buf)); col.cx = 80;
	...
	col.mask = LVCF_WIDTH | LVCF_TEXT; col.pszText = ((LPWSTR)(nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_type)).m_buf)); col.cx = 80;
	...
	col.mask = LVCF_WIDTH | LVCF_TEXT; col.pszText = ((LPWSTR)(nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_value)).m_buf)); col.cx = 200;
	...
	col.mask = LVCF_WIDTH | LVCF_TEXT; col.pszText = ((LPWSTR)(nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_note)).m_buf)); col.cx = 200;
...
[[nodiscard]] static inline mk_view_wstr_t mkfw_wnd_get_del_question(mk_wnd_t* const self)
{
	...
	fmt = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_delete_fmt));
	buf = &g_app.m_tmp_wstrs[g_app.m_tmps_wstr_idx++ % _countof(g_app.m_tmp_wstrs)][0];
	cap = _countof(g_app.m_tmp_wstrs[0]);
...
static inline void mkfw_block_exe(mk_fw_t* const fw, LPCWSTR const path_buf, int const path_len, bool* const gud)
{
	...
	*gud = false;
	...
	len = g_app.m_funcs_ntdll.m_pfn_swprintf(name_ipv4, nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_fmt_block_ipv4_name)).m_buf, exe_name); if(!(len >= 1 && len < nt_path_cap)){ return; }
	len = g_app.m_funcs_ntdll.m_pfn_swprintf(desc_ipv4, nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_fmt_block_ipv4_desc)).m_buf, path_buf); if(!(len >= 1 && len < nt_path_cap)){ return; }
	...
	len = g_app.m_funcs_ntdll.m_pfn_swprintf(name_ipv6, nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_fmt_block_ipv6_name)).m_buf, exe_name); if(!(len >= 1 && len < nt_path_cap)){ return; }
	len = g_app.m_funcs_ntdll.m_pfn_swprintf(desc_ipv6, nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_fmt_block_ipv6_desc)).m_buf, path_buf); if(!(len >= 1 && len < nt_path_cap)){ return; }
...
static inline void mkfw_wnd_insert(mk_wnd_t* const self)
{
	...
	name.lpstrFilter = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_open_filter)).m_buf;
	name.lpstrFile = path_buf;
	name.nMaxFile = path_cap;
	name.lpstrTitle = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_open_title)).m_buf;
	...
	if(b && name.lpstrFile && name.lpstrFile[0] != L'\0')
	{
		...
		if(success)
		{
			res = g_app.m_funcs_user.m_pfn_MessageBoxW(self->m_hwnd, nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_block_added)).m_buf, nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_fire_wall)).m_buf, MB_OK | MB_ICONINFORMATION); ((void)(res));
		}
	}
}
...
static inline void mkfw_wnd_proc__notify_entries___keydown_del(mk_wnd_t* const self, HWND const hwnd, UINT const msg, WPARAM const wparam, LPARAM const lparam, bool* const out_call_def, LRESULT* const out_lr)
{
	...
	question_caption = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_delete_caption));
	...
	if(res == IDYES)
	{
		...
		if(dw == ERROR_SUCCESS)
		{
			ok_text = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_delete_ok_text));
			ok_caption = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_delete_ok_caption));
...
extern "C" DWORD __stdcall mk_entry(PPEB const peb)
{
	...
	wnd_cls_info.lpszClassName = nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_mkfw)).m_buf;
	...
	hwnd = g_app.m_funcs_user.m_pfn_CreateWindowExW(WS_EX_APPWINDOW, ((LPCWSTR)(wnd_cls_atom)), nstr_to_wstr(k_strings.get_nstr(k_strings.string_id::id_fire_wall)).m_buf, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, g_app.m_dlls.m_exe, &g_app.m_fw_wnd); mk_assert(hwnd);
